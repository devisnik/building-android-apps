...
On the "Target Android Devices" screen, select _Phone and Tablet_ and choose any recent API level from the _Minimum SDK_ drop-down list. The figure shows API 19, which is common, but the value chosen will not affect the rest of this guide.
...
* The value of `versionCode` should be an integer that is incremented before uploading a new version of the app into the Google Play store. This value, along with the `applicationId`, tells Google that this is a new version of an existing app, as opposed to a new app.
...
This will run many tasks and eventually return "Build Successful". To see the resulting APK (Android package, the deployable version of an Android app), look in the directory `app/build/outputs/apk`. There you will find a `debug` and a `release` directory.
